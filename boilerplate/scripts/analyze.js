...
gulp.task('analyze:external:lighthouse:open-report', (cb) => {
  openReport(path.join(process.cwd(), 'build', global.settings.platform, 'progressive_web_app_report.html'), cb);
});

gulp.task('analyze:external:pagespeed:open-report', (cb) => {
  openReport(path.join(process.cwd(), 'build', global.settings.platform, 'page_speed_report.html'), cb);
});
