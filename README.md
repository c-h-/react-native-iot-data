# React Native IoT Data App
> This app grabs BLE Motion packets, caches them, analyzes them using a TensorFlow Machine Learning model, and persists the data to the cloud.

## Project Structure
The project has been forked from [Universal Native Boilerplate](https://github.com/c-h-/universal-native-boilerplate) (see readme from that fork below). 

### Top Level Directories
Each deployable platform has platform-specific source code in its respective top level directories (android, ios, macos, server, web, windows).

The `__mocks__` and `__tests__` folders have to do with Jest testing. The `boilerplate` folder contains scripts relating to the boilerplate this project was forked from. The `build` folder aggregates builds by platform and release mode. The `jest` folder contains Jest tests. The `js` folder contains cross-platform source JS. `node_modules` is libraries consumed by the project's JS source or its tooling.

### Import Guidelines
This project is organized to use absolute imports to preserve sanity in the codebase and allow for more easily re-organizing. When importing a file, you typically want to use an absolute path, like `import stuff from 'Common/components/Icon';`. This will go to `~/js/Common/components/Icon` to resolve the import regardless of where the import line is relatively.

It is ok to use relative imports for child components of a component. For example, the AppNavigator has many components involved. Most of those components only apply as child components and would never be consumed outside of the AppNavigator component. For that reason, those child components are grouped into a folder for the AppNavigator component and imported relatively.

# Universal Native Boilerplate (Readme from upstream)
Forked at SHA db09ff7f44b4ae17bbe67248662feb1739d0a8d6
